    ...
    {activeCard && (
      <div className="chudi-screen" onClick={handleClose}>
        <div className="chudi-content">
          ...
          <p className="chudi-desc"><button onClick={() => handleOrderClick(activeCard)} className='saree1-btn'>Order now</button>{activeCard.desc}</p>
        </div>
      </div>
    )}
    {showForm && (
     ...
     <div className='v-btn'>
      <Link to="/views1"><button>View All</button></Link>
     </div>
  </>
  )
}

export default Chudi1
